...
bool ModelLoader::LoadModel(QString filePath, PathType pathType,ModelType modelType){
    ...
    if(scene->HasMeshes())
    {
        for(unsigned int ii=0; ii<scene->mNumMeshes; ++ii)
        {
           ...
           // qDebug() << m_meshes.at(m_meshes.size()-1)->name << " " << m.data()->indexCount;
        }
    }
    else
    {
...
QSharedPointer<Mesh> ModelLoader::processMesh(aiMesh *mesh)
{
    ...
    qDebug() << newMesh->name << " " << newMesh->indexCount;
...
void ModelLoader::processNode(const aiScene *scene, aiNode *node, Node *parentNode, Node &newNode)
{
    ...
    {
        QSharedPointer<Mesh> mesh = m_meshes[node->mMeshes[imesh]+lastmesh];

        qDebug() << "NodeasdasdaName" << m_meshes.size() << " "<< lastmesh;
    ...
    for (int ii=0; ii<newNode.meshes.size(); ++ii) {
        ...
        qDebug() << "    MeshVertices" << newNode.meshes[ii].data()->indexCount;
